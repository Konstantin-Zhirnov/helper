...
export class PostsService {
  ...
  async create(
    createPostDto: CreatePostDto & { images: string[] },
    fieldName: string,
  ...
  async getLocations(): Promise<unknown[]> {
    const locations = await this.postModel
      .find()
  ...
  async removeImage(removeImageDto: RemoveImageDto): Promise<Post> {
    ...
    return this.postModel.findByIdAndUpdate(
      removeImageDto._id,
      { images: post.images.filter((image) => image !== removeImageDto.image) },
      { new: true },
    );
  }
}
