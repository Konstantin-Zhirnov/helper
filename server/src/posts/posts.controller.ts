...
export class PostsController {
  ...
  @Get('posts')
  async getAll(@Query() { location, category, search, page }) {
  ...
  async findOne(@Param('id') id: string): Promise<PostModel> {
    return await this.postsService.findOneAndPopulate(
      id,
      'authorId',
      chosenFields,
    );
  ...
  @UseInterceptors(FilesInterceptor('images', 5, localOptions) as any)
  async create(
  ...
  @Put('update-post')
  async update(@Body() updatePostDto: UpdatePostDto): Promise<UpdatePostDto> {
  ...
  async getAllPostsById(@Param('id') id: string): Promise<PostModel[]> {
    return await this.postsService.getAllPostsByAuthorId(
      id,
      'authorId',
      chosenFields,
    );
  ...
  @ApiResponse({ status: 201, type: PostModel })
  @Post('remove-image')
  async removeImage(@Body() removeImageDto: RemoveImageDto) {
